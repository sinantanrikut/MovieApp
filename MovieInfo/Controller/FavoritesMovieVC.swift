...
class FavoritesMovieVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: WATCH_LIST_ID1, for: indexPath) as? favoritescell{
    ...
    func fetchObject(){
        ...
        let refDB = Database.database().reference(withPath: "Favorites")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
      ...
      //  moviedetail = viewModel.self
        performSegue(withIdentifier: FAVORITE_TO_DETAIL, sender: nil)
